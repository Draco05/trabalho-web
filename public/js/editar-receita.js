/**
 * js/editar-receita.js
 * Carrega uma receita da API pelo id na URL,
 * preenche o formulário e salva as alterações via API.
 */
...
(function () {
  ...
  const msgErroCarregar = document.getElementById('msg-erro-carregar');
  ...
  async function carregarReceita() {
    ...
    receitaId = params.get('id');

    if (!receitaId) {
      msgErroCarregar.textContent   = 'Nenhuma receita especificada.';
      msgErroCarregar.style.display = 'block';
      return;
    }

    var receita;
    try {
      receita = await RF.storage.buscarReceitaPorId(receitaId);
    } catch (err) {
      msgErroCarregar.textContent   = 'Receita não encontrada ou acesso negado.';
      msgErroCarregar.style.display = 'block';
      return;
    }

    preencherFormulario(receita);
    conteudoEdicao.style.display = 'block';
  }

  function preencherFormulario(receita) {
    nomeInput.value      = receita.nome        || '';
    descInput.value      = receita.descricao   || '';
    tempoInput.value     = (receita.tempo_preparo || receita.tempoPreparo) > 0
                            ? (receita.tempo_preparo || receita.tempoPreparo) : '';
    porcoesInput.value   = receita.porcoes     > 0 ? receita.porcoes : '';
    ...
    if (imgSrc) {
      ...
      labelUpload.textContent  = '✔ Imagem carregada — clique para trocar';
    }

    ingredientes = (receita.ingredientes || []).slice();
    etapas       = (receita.etapas       || []).slice();
    renderIngredientes();
    renderEtapas();
  }
  ...
  function renderIngredientes() {
    ...
    listaIngredientes.querySelectorAll('[data-idx]').forEach(btn =>
  ...
  btnAddIng.addEventListener('click', function () {
    ...
    if (!nome) { mostrarErro('erro-ingrediente', 'Informe o nome.'); ingNomeInput.focus(); return; }
    if (!qtd)  { mostrarErro('erro-ingrediente', 'Informe a quantidade.'); ingQtdInput.focus(); return; }
    ingredientes.push({ nome, quantidade: qtd });
    ingNomeInput.value = ''; ingQtdInput.value = ''; ingNomeInput.focus();
  ...
  function renderEtapas() {
    ...
    listaEtapas.querySelectorAll('[data-idx]').forEach(btn =>
      btn.addEventListener('click', function () {
  ...
  btnAddEtapa.addEventListener('click', function () {
    ...
    if (!desc) { mostrarErro('erro-etapa', 'Informe a descrição.'); etapaDescInput.focus(); return; }
    etapas.push({ numero: etapas.length + 1, descricao: desc });
    etapaDescInput.value = ''; etapaDescInput.focus();
  ...
  function validar() {
    let ok = true;
    if (!nomeInput.value.trim())   { mostrarErro('erro-nome',               'O nome é obrigatório.'); ok = false; }
  ...
  btnSalvar.addEventListener('click', async function () {
    ...
    const dados = {
      id:           receitaId,          // presença de id → PUT no storage.js
    ...
    try {
      ...
      mensagemSucesso.textContent   = '✔ Alterações salvas com sucesso! Redirecionando...';
      mensagemSucesso.style.display = 'block';
      mensagemSucesso.scrollIntoView({ behavior: 'smooth', block: 'center' });
      setTimeout(() => { window.location.href = 'receita.html?id=' + encodeURIComponent(receitaId); }, 2000);
    } catch (err) {
      mensagemGlobal.textContent   = 'Erro ao salvar: ' + (err.message || 'tente novamente.');
      mensagemGlobal.style.display = 'block';
